...
def load_generator(gen, ckpt_path, ex_id, ep_id):

    base_ckpt_path = os.path.dirname(os.path.dirname(ckpt_path)) + '/'
    ckpt_path_gen = base_ckpt_path + ex_id + '/generator/' + ep_id

    if os.name == 'nt':
        ckpt_path_gen = 'C:\\Users\\tuk\\Documents\\Uni-Due\\Bachelorarbeit\\dir_working\\scrabble-gan\\data\\scrabble-gan-checkpoints\\final60\\generator\\95'

    # load gen
    latest_checkpoint_gen = tf.train.latest_checkpoint(ckpt_path_gen)
    # if model must be restored (for inference), there must be a snapshot
    if not latest_checkpoint_gen:
        raise Exception('No saved model found in: ' + ckpt_path_gen)
    load_status = gen.load_weights(latest_checkpoint_gen)
    load_status.assert_consumed()
    print("Gen. Model from " + ckpt_path_gen + " loaded")
