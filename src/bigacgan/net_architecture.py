...
def make_generator(latent_dim, input_dim, embed_y, kernel_reg, blocks_with_attention, vocab_size,
    ...
    imgs = layers.Input(shape=[input_dim[0], input_dim[1], input_dim[2]])
    ...
    # ResNetBlock "down"
    for block_idx in range(num_blocks2):
        name = "B_style{}".format(block_idx + 1)
        is_last_block = block_idx == num_blocks2 - 1
        net = ResNetBlockDown(name, out_channels_style[block_idx], is_last_block, kernel_reg).call(net)
        if name == "B_style1":
    ...
    net = tf.nn.tanh(net)
    # define model
    model = tf.keras.Model([imgs, y], net)
...
def make_style_promoter(input_dim, kernel_reg, blocks_with_attention, vis_model=True):
    """
     (fully convolutional) Discriminator based on
...
def make_gan(g_model, d_model, r_model, w_model, vis_model=True):
    """
    Update G through composite model
    ...
    # run D + R on image generated by G
    gan_output_d = d_model(g_model.output, training=True)
    gan_output_w = w_model(g_model.output, training=True)
    gan_output_r = r_model([g_model.output, g_model.input[1], input_length, label_length], training=True)

    # define composite model
    model = tf.keras.Model(g_model.input + [input_length, label_length], [g_model.output, gan_output_d, gan_output_r, gan_output_w])
